    ...
    return {
        ...
        optional: {
            ...
            registrar: function (registrar) {
                ...
                var parsed = Grammar.URIParse(registrar);
                if (!parsed) {
                    return;
                }
                else if (parsed.user) {
...
var RegisterContext = /** @class */ (function (_super) {
    ...
    function RegisterContext(ua, options) {
        ...
        _this = _super.call(this, ua, C.REGISTER, settings.registrar, settings) || this;
        _this.type = TypeStrings.RegisterContext;
            ...
            switch (true) {
                ...
                case /^2[0-9]{2}$/.test(statusCode):
                    ...
                    _this.registrationExpiredTimer = setTimeout(function () {
                        ...
                        if (_this.registered) {
                            _this.unregistered(undefined, C.causes.EXPIRES);
                        }
                    }, expires * 1000);
                    // Save gruu values
                    if (contact.hasParam("temp-gruu")) {
                        _this.ua.contact.temp_gruu = Grammar.URIParse(contact.getParam("temp-gruu").replace(/"/g, ""));
                    }
                    if (contact.hasParam("pub-gruu")) {
                        _this.ua.contact.pub_gruu = Grammar.URIParse(contact.getParam("pub-gruu").replace(/"/g, ""));
                    }
                ...
                case /^423$/.test(statusCode):
                    if (response.hasHeader("min-expires")) {
                        ...
                        _this.registrationFailure(response, C.causes.SIP_FAILURE_CODE);
                    }
                    break;
                default:
                    _this.registrationFailure(response, Utils.sipErrorCause(response.statusCode || 0));
            }
        };
        this.onRequestTimeout = function () {
            _this.registrationFailure(undefined, C.causes.REQUEST_TIMEOUT);
        };
        this.onTransportError = function () {
            _this.registrationFailure(undefined, C.causes.CONNECTION_ERROR);
        };
        this.cseq++;
        if (this.request) {
        ...
        this.receiveResponse = function (response) {
            ...
            switch (true) {
                ...
                case /^2[0-9]{2}$/.test(statusCode):
                    ...
                    break;
                default:
                    _this.unregistered(response, Utils.sipErrorCause(response.statusCode || 0));
            }
        };
        this.onRequestTimeout = function () {
