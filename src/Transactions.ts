...
export function checkTransaction(ua: UA, request: IncomingRequest): boolean {
  const inviteServertr: InviteServerTransaction | undefined = ua.transactions.ist[request.viaBranch];
  switch (request.method) {
    ...
    default:
      ...
      const nist: NonInviteServerTransaction | undefined = ua.transactions.nist[request.viaBranch];
      if (nist) {
        switch (nist.state) {
          case TransactionStatus.STATUS_TRYING:
            break;
