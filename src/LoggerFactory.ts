...
export class LoggerFactory implements LoggerFactoryDefinition {
  ...
  // tslint:disable-next-line:variable-name
  private _connector: ((level: string, category: string, label: string, content: any) => void) | undefined;
  ...
  get connector(): ((level: string, category: string, label: string, content: any) => void) | undefined {
    return this._connector;
  }
  set connector(value: ((level: string, category: string, label: string, content: any) => void) | undefined) {
    if (!value) {
  ...
  public print(target: ((content: string) => void), category: string, label: string, content: any): void {
  ...
  private genericLog(levelToLog: Levels, category: string, label: string, content: any): void {
    if (this.level >= levelToLog) {
      if (this.builtinEnabled) {
        this.print((console as any)[Levels[levelToLog]], category, label, content);
      }
...
export class Logger implements LoggerDefinition {
  ...
  private genericLog(level: Levels, content: string): void {
    (this.logger as any)[Levels[level]](this.category, this.label, content);
  }
}
